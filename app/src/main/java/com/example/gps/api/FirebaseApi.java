...
public class FirebaseApi {
    ...
    // 추천 코스 목록 가져오기
    public static CompletableFuture<List<CourseRecommendation>> getRecommendedCourses(String season, String weather, int crowdLevel) {
        CompletableFuture<List<CourseRecommendation>> future = new CompletableFuture<>();
        
        db.collection("recommendations")
            .whereEqualTo("season", season)
            .whereEqualTo("weather", weather)
            .whereGreaterThanOrEqualTo("crowdLevel", crowdLevel - 1)
            .whereLessThanOrEqualTo("crowdLevel", crowdLevel + 1)
            .limit(10)
            .get()
            .addOnSuccessListener(querySnapshot -> {
                ...
                for (DocumentSnapshot doc : querySnapshot.getDocuments()) {
                    ...
                    rec.setCourseId(doc.getLong("courseId").intValue());
                    rec.setCourseName(doc.getString("courseName"));
                    rec.setRecommendationScore(doc.getDouble("recommendationScore"));
                    rec.setReason(doc.getString("reason"));
                    rec.setSeason(doc.getString("season"));
                    rec.setWeather(doc.getString("weather"));
                    rec.setCrowdLevel(doc.getLong("crowdLevel").intValue());
                    rec.setFestival(doc.getBoolean("isFestival"));
                    
                    if (rec.isFestival()) {
                        rec.setFestivalName(doc.getString("festivalName"));
                        rec.setFestivalPeriod(doc.getString("festivalPeriod"));
                    }
